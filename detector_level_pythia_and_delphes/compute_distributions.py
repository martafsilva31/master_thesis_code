    ...
    for i_panel, (i_obs, xlabel) in enumerate(zip(observable_indices, observable_labels)):
        ...
        for i_theta, theta_matrix in enumerate(theta_matrices):
            ...
            if sample_only_from_closest_benchmark:
                ...
                histo, bin_edges = np.histogram(
                    ...
                    bins=n_bins,
                    range=x_ranges[i_panel],
                    weights=indiv_theta_weights,
                    density=normalize,
                )
            else:
                histo, bin_edges = np.histogram(
                    all_x[:, i_obs], bins=n_bins, range=x_ranges[i_panel], weights=theta_weights, density=normalize
                )
            histos.append(histo)
        ...
        for i_theta, theta_matrix in enumerate(theta_matrices_bkgs):
            ...
            if sample_only_from_closest_benchmark:
                ...
                histo, bin_edges = np.histogram(
                    all_x_bkgs[:, i_obs], bins=n_bins, range=x_ranges[i_panel], weights=theta_weights, density=normalize
                )
            histos.append(histo)
        ...
        plt.xlim(x_ranges[i_panel][0], x_ranges[i_panel][1])
        if log:
...
if __name__ == '__main__':
    ...
    for channel in args.channel:
        ...
        if not args.plot_sally:
            ...
            parameter_points=['sm',np.array([1.150]),np.array([-1.035]),] if args.sample_type!='backgroundOnly' else [],
            ...
            line_labels=['SM',r'$c_\tilde{HW} = 1.150$',r'$c_\tilde{HW} = -1.035$'] if args.sample_type!='backgroundOnly' else [],
            log=args.do_log,
    ...
    #     filename='/lstore/titan/martafsilva/master_thesis/master_thesis_output/detector_level_pythia_and_delphes/samples_possible_bug_combiner/wh_signalh5',
    #     parameter_points=['sm',np.array([1.150]),np.array([-1.035])],
    #     line_labels=['SM',r'$c_\tilde{HW} = 1.150$',r'$c_\tilde{HW} = -1.035$'],
    #     uncertainties='none',
    #     n_bins=20,
    #     normalize=True,
    #     sample_only_from_closest_benchmark=True
    ...
    # plt.savefig('distributions_sem_BSM.pdf')
